...
var palettes = {
  "正常":          ['#000000', '#444444', '#999999', '#ffffff', '#603913', '#c69c6d',
                      '#ffdab9', '#ff0000', '#ffd700', '#ff6600', '#16ff00', '#0fad00',
                      '#00ffff', '#0247fe', '#ec008c', '#8601af', '#fffdc9'],
  "乌贼":           ['#402305', '#503315', '#604325', '#705335', '#806345', '#907355',
                      '#a08365', '#b09375', '#bfa284', '#cfb294', '#dfc2a4', '#ffe2c4'],
  "灰度":       ['#000000', '#111111', '#222222', '#333333', '#444444', '#555555', '#666666',
  ...
  "氖？":            ['#ffffff', '#000000', '#adfd09', '#feac09', '#fe0bab', '#ad0bfb', '#00abff'],
  "感恩":    ['#673718', '#3c2d27', '#c23322', '#850005', '#c67200', '#77785b',
                      '#5e6524', '#cfb178', '#f5e9ce'],
  "假日":         ['#3d9949', '#7bbd82', '#7d1a0c', '#bf2a23',
                      '#fdd017', '#00b7f1', '#bababa', '#ffffff'],
  "情人":     ['#8b2158', '#a81f61', '#bb1364', '#ce0e62', '#e40f64', '#ff0000',
                      '#f5afc8', '#ffccdf', '#e7e7e7', '#ffffff'],
  "万圣节":       ['#444444', '#000000', '#999999', '#ffffff', '#603913', '#c69c6d',
                      '#7a0e0e', '#b40528', '#fd2119', '#fa5b11', '#faa611', '#ffd700',
                      '#602749', '#724b97', '#bef202', '#519548', '#b2bb1e'],
  "蓝调":       ['#b6cbe4', '#618abc', '#d0d5ce', '#82a2a1', '#92b8c1', '#607884',
                      '#c19292', '#8c2c2c', '#295c6f'],
  "春天":          ['#9ed396', '#57b947', '#4d7736', '#365431', '#231302',
                      '#3e2409', '#a66621', '#a67e21', '#ebbb49', '#ffc0cb', '#ffffff'],
  "海滩":           ['#1ca4d2', '#65bbe2', '#6ab7bf', '#94cbda', '#9cbf80', '#d2e1ab',
                      '#b8a593', '#d7cfb9', '#dc863e', '#f7dca2'],
  "2016年的颜色":  ['#91a7d0', '#f6cac9', '#eb9587', '#776a5f',
                      '#d1c2ab', '#a39d9d', '#648589'],
  "蜜蜂":             ["#000000", "#7a5c00", "#b58800", "#eab618", "#f6de97", "#ffffff"],
  "2017年的颜色":  ['#86af49', '#44883d', '#1f4478', '#0062a3',
                      '#00939a', '#59c9d5', '#8a9a9a', '#5f7278'],
  "冰与火":    ['#520909', '#b40528', '#fd2119', '#faa611', '#ffe96a',
                      '#ffffff', '#69ddff', '#1c8ae5', '#0a3fa9', '#040526'],
  "峡谷日落":   ['#fce3ca', '#feb789', '#f27c8a', '#af5081',
                      '#8e6dae', '#5f4a8b', '#2e1b50'],
  "多汁":           ["#f3ab54", "#ec5e66", "#ab5871", "#f2a19b", "#f9f4d4", "#fadfb7",
                      "#869e3c", "#cbdd7e", "#fced95"],
  "热点":        ["#f68357", "#fbc040", "#fefa56", "#fef0f5", "#90fc51", "#07f182",
                      "#1d6ab2", "#12041b", "#2f0946"],
  "黎明使者16":  ['#140c1c', '#442434', '#30346d', '#4e4a4e', '#854c30', '#346524',
                      '#d04648', '#757161', '#597dce', '#d27d2c', '#8595a1', '#6daa2c',
                      '#d2aa99', '#6dc2ca', '#dad45e', '#deeed6'],
  "阿恩":            ['#000000', '#9d9d9d', '#ffffff', '#be2633', '#e06f8b', '#493c2b',
                      '#a46422', '#eb8931', '#f7e26b', '#2f484e', '#44891a', '#a3ce27',
                      '#1b2632', '#005784', '#31a2f2', '#b2dcef'],
  "巴纳诺多尔":      ['#000000', '#454541', '#aaaaaa', '#dddddd', '#5668ac', '#7999da',
                      '#a0cdf0', '#ffffff', '#214e3f', '#60997b', '#c0e48c', '#f5f0d0',
                      '#954961', '#b98f7b', '#f3a3c0', '#ebcb71'],
  // 必须永远在最后
  "Custom":          ['#000000', '#2c3539', '#2b3856', '#002a6c', '#800080', '#a52a2a',
...
function bindEvents()
{
  ...
  var highlightForCopying = function(e)
  {
    this.select();
  };
  ID("imgur").addEventListener('click', function(e)
  {
    ...
    anbt.UploadToImgur(function(r)
    {
      ...
      ID("popuptitle").innerHTML = "Imgur upload result";
      ...
      if (r && r.success)
      {
        ...
        ID("popupcontents").innerHTML =
          '<a id="imgururl" class="prominent">Uploaded image</a><br>' +
          '<a id="imgurdelete" target="_blank">Imgur delete</a><hr>' +
          'Code for copying:<br>' +
          '<textarea id="copytext"></textarea>';
        ID("copytext").value = "[![image description](https://i.imgur.com/" + r.data.id + ".png)](" + location.href +")\n";
        ID("copytext").addEventListener("click", highlightForCopying);
        ID("imgururl").href = "https://imgur.com/" + r.data.id;
        ID("imgurdelete").href = "https://imgur.com/delete/" + r.data.deletehash;
      } else {
        var err = r.data ? ("Imgur error: " + r.data.error) : ("Error: " + r);
        ID("popupcontents").innerHTML =
          '<div id="imgurerror">' + err.replace(/</g, "&lt;") + '</div>';
      }
      ID("imgur").disabled = false;
    });
  });
  ID("sharepalette").addEventListener('click', function(e)
  {
    e.preventDefault();
    ...
    canvas.width = 18 * palettes.Custom.length + 2;
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.globalAlpha = 0.5;
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.globalAlpha = 1;
    for (var j = 0; j < palettes.Custom.length; j++)
    {
      ctx.fillStyle = palettes.Custom[j];
      ctx.fillRect(j * 18 + 1, 1, 18, 18);
    }

    var imgdata = canvas.toDataURL();
    var url = location.href.replace(/(#.+)|$/, paletteToString());
    ID("popup").classList.add("show");
    ID("popuptitle").innerHTML = "分享自定义调色板";
    ID("popupcontents").innerHTML =
      '<img src="' + imgdata + '"><hr>' +
      'Code for copying:<br>' +
      '<textarea id="copytext"></textarea>';
    ID("copytext").value = "[![Custom palette](" + imgdata + ")](" + url +")\n";
    ID("copytext").addEventListener("click", highlightForCopying);
  ...
  {
    ID("brush" + i).addEventListener('mousedown', makeBrushFunc(brushSizes[i]), false);
  ...
  var updateColorIndicators = function()
  {
    ...
    if (c0 == 'eraser')
    {
      ...
      //var hsl = HUSL.fromHex(color2hex(c0));
      //ID("hue").value = hsl[0];
      //ID("saturation").value = hsl[1];
      //ID("lightness").value = hsl[2];
    }
    if (c1 == 'eraser')
    {
  ...
  var editColors = false;
  var updateEditColors = function(b)
  {
    editColors = b;
    if (b)
    {
      ID("colors").classList.add("edit");
      ID("indicator").classList.add("edit");
    } else {
      ID("colors").classList.remove("edit");
      ID("indicator").classList.remove("edit");
    }
  };
  var colorClick = function(e)
  {
    if (e.touches || e.button === 0 || e.button === 2)
    {
      ...
      if (editColors && e.button !== 2 && getPointerType() !== 3)
      {
        if (this.id == "eraser")
        {
          updateEditColors(false);
        } else {
          //TODO: edit colors visually
          var hexcolor = color2hex(color);
          var coloridx = anbt.palette.indexOf(hexcolor);
          var m = prompt("New color "+(coloridx+1)+" :", hexcolor);
          if (m)
          {
            var newcolor = color2hex(m);
            this.style.backgroundColor = newcolor;
            anbt.palette[coloridx] = newcolor;
            updateCustomPalette()
          }
        }
      }
  ...
  var setPalette = function(name, edit)
  {
    ...
    for (var i = 0; i < colors.length; i++)
    {
      ...
      b.addEventListener('touchstart', colorClick);
  ...
  var paletteFromText = function(data)
  {
    //alert("pal from hash: \n" + data);
    palettes.Custom = data ? data.split(";").map(function(v){return '#'+v}) : [];
    setPalette("Custom", !data);
  };
  window.paletteFromText = paletteFromText;
  var paletteToString = function()
  {
    return "#pal/" + palettes.Custom.join(";").replace(/#/g, "");
  };
  var paletteToHash = function()
  {
    var data = paletteToString();
    history.replaceState(null, null, data);
  };
  var updateCustomPalette = function(e)
  {
    var e;
    var chooser = ID("palettechooser");
    if (anbt.palette == palettes.Custom)
    {
      e = chooser.querySelector("div[data-name='Custom']");
      if (e) chooser.removeChild(e);
    } else {
      // Force rebuild whole list, just in case
      while (e = chooser.childNodes[0]) chooser.removeChild(e);
    }
    paletteToHash();
  };
  ...
  {
    e.preventDefault();
    if (anbt.palette.length >= 17) return;
    var color = color2hex(anbt.color[0]);
    anbt.palette.push(color);
    var b = document.createElement("b");
    b.style.backgroundColor = color;
    b.addEventListener('mousedown', colorClick);
    b.addEventListener('touchstart', colorClick);
    b.addEventListener('contextmenu', noDefault);
    ID("palette").insertBefore(b, ID("eraser"));
    updateCustomPalette();
  };
  var delColor = function(e)
  {
    e.preventDefault();
    if (!anbt.palette.length) return;
    var coloridx = -1;
    if (anbt.color[1] != "eraser")
    {
      var color = color2hex(anbt.color[1]);
      // remove the last occurence of secondary color from
      // the palette if found, else remove the last one
      var coloridx = anbt.palette.lastIndexOf(color);
    }
    if (coloridx == -1)
    {
      coloridx = anbt.palette.length - 1;
      anbt.SetColor(1, anbt.palette[coloridx]);
      updateColorIndicators();
    }
    anbt.palette.splice(coloridx, 1);
    var pal = ID("palette");
    var els = pal.querySelectorAll("b");
    pal.removeChild(els[coloridx]);
    updateCustomPalette();
  };
  ID("addcolor").addEventListener('mousedown', addColor);
  ID("addcolor").addEventListener('touchstart', addColor);
  ID("delcolor").addEventListener('mousedown', delColor);
  ID("delcolor").addEventListener('touchstart', delColor);
  var maybeAddColor = function(e)
  {
    if (!editColors) return;
    addColor(e);
  };
  var maybeDelColor = function(e)
  {
    if (!editColors) return;
    delColor(e);
  };
  ID("primary").addEventListener('mousedown', maybeAddColor);
  ID("primary").addEventListener('touchstart', maybeAddColor);
  ID("secondary").addEventListener('mousedown', maybeDelColor);
  ID("secondary").addEventListener('touchstart', maybeDelColor);
  ...
  var knobCommonMove = function(e)
  {
    ...
    if (e.touches)
    {
      x = e.touches[0].pageX - rect.left /*- pageXOffset*/ - 34;
    } else {
      x = e.pageX - rect.left - pageXOffset - 34;
    }
  ...
  var closePaletteList = function(e)
  {
    if (e.touches || e.button === 0)
    {
      ...
      window.removeEventListener('touchstart', closePaletteList);
    }
  };
    ...
    if (e.touches || e.button === 0)
    {
      ...
      if (chooser.classList.contains("open"))
      {
        setTimeout(function()
        {
          ...
          window.addEventListener('touchstart', closePaletteList);
        }, 1);
      }
      ...
      if (chooser.childNodes.length < keys.length)
      {
        ...
        for (var i = chooser.childNodes.length; i < keys.length; i++)
        {
          ...
          var div = document.createElement("div");
          div.setAttribute("data-name", keys[i]);
          ...
          div.addEventListener('touchstart', choosePalette);
  ...
  ID("palettename").addEventListener('touchstart', openPaletteList);
  ...
  ID("openmenu").addEventListener('touchstart', openMenu);
  ...
  ID("simplay").addEventListener('touchstart', simulatePlay);
  ...
  var exportCustomPNG = function(e)
  {
    e.preventDefault();
    if (warnStrokesAfterPos()) return;
    var m = prompt("导出的PNG尺寸? 仅输入宽度以保持纵横比。", "600x500");
    if (!m) return;
    m = m.match(/(\d+)/g);
    if (!m)
    {
  ...
  ID("custompng").addEventListener('touchstart', exportCustomPNG);
  ...
  ID("transparentbg").addEventListener('touchstart', setTransparentBackground);
  ...
  var enablePatterns = function(e)
  {
    ...
    ID("enablepatterns").removeEventListener('touchstart', enablePatterns);
    var div = document.createElement('div');
    div.className = "panel";
  ...
  ID("enablepatterns").addEventListener('touchstart', enablePatterns);
  ...
  ID("exportwebm").addEventListener('touchstart', exportWebM);

  var usageTips = function(e)
  {
    e.preventDefault();
    alert("阅读按钮上的工具提示!\n\n" +
      "按Alt键选择颜色(按Shift键保留不透明度)\n" +
      "按T将原色不透明度减半\n" +
      "按X可交换颜色\n\n" +
      "在触摸屏上:\n" +
      "将一根手指放在左边，用第二根手指轻击即可撤消，\n用右手轻击即可重做");
  }
  ID("usagetips").addEventListener('mousedown', usageTips);
  ID("usagetips").addEventListener('touchstart', usageTips);

  var drawTransitions = function(e)
  {
    e.preventDefault();
    if (anbt.unsaved && !confirm("你还没有保存画作. 继续?")) return;
  ...
  ID("drawtransitions").addEventListener('touchstart', drawTransitions);
  

  var putImageForColorpicking = function(url)
  {
    if (!url.match(/^palettes\//) && !url.match(/^https?:\/\/i\.imgur\.com\//))
    {
      alert("Sorry, can only put images from imgur.com");
      return;
    }
    anbt.SetBackground(anbt.palette[0]);
    anbt.ctx.clearRect(0, 0, 600, 500);
    ...
    img.onload = function()
    {
      ...
      anbt.ctx.drawImage(this, 0, 0, w, h);
    };
    img.src = url;
    window.img = img;
  };
  window.putImageForColorpicking = putImageForColorpicking;

  var setBackgroundImage = function(e)
  {
    e.preventDefault();
    if (anbt.unsaved && !confirm("你没有保存画作. 继续?")) return;
    anbt.SetBackground("eraser");
    ID("svgContainer").style.backgroundImage = 'url(' + prompt('输入用作背景图像的图像URL:') + ')';
    ID("svgContainer").style.backgroundSize = '100%';
  };
  ID("setbackgroundimage").addEventListener('mousedown', setBackgroundImage);
  ID("setbackgroundimage").addEventListener('touchstart', setBackgroundImage);
  ...
  var toggleSmooth = function(e)
  {
    ...
    ID("togglesmooth").childNodes[0].nodeValue =
      (window.toggleSmooth ? "启用" : "禁用") + "描边平滑";
  };
  ID("togglesmooth").addEventListener('mousedown', toggleSmooth);
  ID("togglesmooth").addEventListener('touchstart', toggleSmooth);

  var setSnap = function(e)
  {
    e.preventDefault();
    var m = prompt("网格大小(以像素为单位)(6使笔刷大小2和4更精确;0禁用):", "0");
    if (!m) return;
    m = m.match(/(\d+)/g);
    if (!m) return;
  ...
  ID("setsnap").addEventListener('touchstart', setSnap);
      ...
      if (i < els.length)
      {
        ...
        if (editColors && e.button !== 2 && getPointerType() !== 3)
        {
          if (color == "eraser")
          {
            ...
            {
              var newcolor = color2hex(m);
              els[i].style.backgroundColor = newcolor;
              anbt.palette[coloridx] = newcolor;
              updateCustomPalette();
            }
          }
...
function main()
{
  ...
  if (location.hash.length > 2)
  {
    ...
    var m;
    if (m = id.match(/drawception\/(\w{8})/))
    {
      anbt.FromURL("drawception-get-panel.php?panelid=" + m[1]);
    }
    else if (m = id.match(/pal\/([^/]*)/))
    {
      paletteFromText(m[1]);
    }
    else if (m = id.match(/put\/(.+)/))
    {
      putImageForColorpicking(m[1]);
    }
...
if 
  (!navigator.userAgent.match(/\bPresto\b/) // Presto Opera has SVGPathSegList, but doesn't have it in window, also doesn't have MutationObserver
    &&
    (
      !("SVGPathSeg" in window)
      || !("SVGPathSegList" in window) || !("appendItem" in window.SVGPathSegList.prototype) // Damn Firefox 59+!
    )
  )
{
